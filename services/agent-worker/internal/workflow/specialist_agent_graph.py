"""Specialist agent (multi-agent) workflow graph"""
...
from internal.workflow.specialist_agent_nodes import implementing_specialist_node
import asyncio
...
def create_specialist_agent_graph(checkpointer: MemorySaver) -> StateGraph:
    """Create the specialist agent (multi-agent) workflow graph"""
    
    workflow = StateGraph(EngineeringState)
    
    # Add all nodes for specialist agent workflow
    ...
    workflow.add_node("IMPLEMENTING", implementing_specialist_node)
    ...
    workflow.add_conditional_edges(
        "WAITING_APPROVAL",
        lambda state: "FAILED" if state.get("approval_decisions", {}).get("last") != "approved" else "continue",
        {
            "continue": "IMPLEMENTING",
            "FAILED": "FAILED"
        }
...
async def create_specialist_agent_run(state: EngineeringState, checkpointer: MemorySaver) -> Dict[str, Any]:
    """Create and start a specialist agent run"""
