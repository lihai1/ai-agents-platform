...
@pytest.mark.e2e
async def test_complete_go_feature_workflow():
    ...
    async with AsyncClient(base_url="http://localhost:8000") as client:
        ...
        while (datetime.utcnow() - start_time).total_seconds() < max_wait:
            response = await client.get(f"/agent/v1/runs/{run_id}")
            assert response.status_code == 200
...
@pytest.mark.e2e
async def test_workflow_with_cancellation():
    ...
    async with AsyncClient(base_url="http://localhost:8000") as client:
        ...
        # Cancel the run
        response = await client.post(f"/agent/v1/runs/{run_id}/cancel")
        assert response.status_code == 200
...
@pytest.mark.e2e
async def test_workflow_with_budget_exceeded():
...
async def test_event_streaming():
    ...
    async with AsyncClient(base_url="http://localhost:8000") as client:
        ...
        response = await client.get(f"/agent/v1/runs/{run_id}/events")
        assert response.status_code == 200
