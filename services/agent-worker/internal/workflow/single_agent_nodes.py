...
async def reasoning_node(state: EngineeringState) -> EngineeringState:
    ...
    await publish_state_event(state["run_id"], "reasoning", {"status": "REASONING"})
    ...
    if success:
        ...
        await publish_chat_event(state["run_id"], "final_answer", {"content": final_text, "status": "completed"})
    else:
        error_text = reasoning_results.get("answer", f"Task failed: {reasoning_results.get('errors', ['Unknown error'])[0]}")
        errors = reasoning_results.get("errors", [])
        await publish_chat_event(state["run_id"], "final_answer", {"content": error_text, "error": True, "errors": errors, "status": "failed"})
