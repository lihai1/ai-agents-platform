...
from internal.agents.specialist.agents import SkillsLeadAgent, SolutionPlannerAgent, RepoScoutAgent
from internal.agents.specialist.validators import BackendTestEngineerAgent, AngularTestEngineerAgent, CodeReviewerAgent, CompletionVerifierAgent
import os
import logging
...
async def created_node(state: EngineeringState) -> EngineeringState:
    ...
    await publish_state_event(state["run_id"], state["user_id"], "created", {"status": "CREATED"})
    return state
...
async def preparing_workspace_node(state: EngineeringState) -> EngineeringState:
    ...
    await publish_state_event(state["run_id"], state["user_id"], "preparing_workspace", {"status": "PREPARING_WORKSPACE"})
...
async def scouting_node(state: EngineeringState) -> EngineeringState:
    ...
    await publish_state_event(state["run_id"], state["user_id"], "scouting", {"status": "SCOUTING"})
...
async def planning_node(state: EngineeringState) -> EngineeringState:
    ...
    await publish_state_event(state["run_id"], state["user_id"], "planning", {"status": "PLANNING"})
...
async def designing_node(state: EngineeringState) -> EngineeringState:
    ...
    await publish_state_event(state["run_id"], state["user_id"], "designing", {"status": "DESIGNING"})
...
async def testing_node(state: EngineeringState) -> EngineeringState:
    ...
    await publish_state_event(state["run_id"], state["user_id"], "testing", {"status": "TESTING"})
...
async def reviewing_node(state: EngineeringState) -> EngineeringState:
    ...
    await publish_state_event(state["run_id"], state["user_id"], "reviewing", {"status": "REVIEWING"})
...
async def verifying_node(state: EngineeringState) -> EngineeringState:
    ...
    await publish_state_event(state["run_id"], state["user_id"], "verifying", {"status": "VERIFYING"})
...
async def repairing_node(state: EngineeringState) -> EngineeringState:
    ...
    await publish_state_event(state["run_id"], state["user_id"], "repairing", {"status": "REPAIRING", "repair_count": state["repair_count"]})
...
async def waiting_approval_node(state: EngineeringState) -> EngineeringState:
    ...
    await publish_state_event(state["run_id"], state["user_id"], "waiting_approval", {"status": "WAITING_APPROVAL"})
...
async def completed_node(state: EngineeringState) -> EngineeringState:
    ...
    await publish_state_event(state["run_id"], state["user_id"], "completed", {"status": "COMPLETED"})
    return state
...
async def failed_node(state: EngineeringState) -> EngineeringState:
    ...
    await publish_state_event(state["run_id"], state["user_id"], "failed", {"error_message": error_message})
    return state
...
async def cancelled_node(state: EngineeringState) -> EngineeringState:
    ...
    await publish_state_event(state["run_id"], state["user_id"], "cancelled", {"error_message": "Run cancelled by user"})
    return state
...
async def budget_exceeded_node(state: EngineeringState) -> EngineeringState:
    ...
    await publish_state_event(state["run_id"], state["user_id"], "budget_exceeded", {"error_message": "Budget limit exceeded"})
    return state
